...
export function ChatRoomScreen({ route, navigation }: any) {
  ...
  const handleSend = async () => {
    ...
    const optimisticMsg: Message = {
      ...
      content,
      senderId: myUserId,
      sender: { id: myUserId, email: user?.email, role: 'CUSTOMER' },
  ...
  const renderMessage = ({ item }: { item: Message }) => {
    ...
    // 1. If sender role is CUSTOMER, customer is viewing this app -> isMe is TRUE
    // 2. If senderId matches user's ID -> isMe is TRUE
    // 3. If sender role is FIXER -> isMe is FALSE
    let isMe = false;
    if (item.sender?.role === 'CUSTOMER') {
      isMe = true;
    } else if (item.sender?.role === 'FIXER') {
      isMe = false;
    ...
    const partnerDisplayName = otherUserName || (item.sender?.email ? item.sender.email.split('@')[0] : 'Fixer');
        ...
        <View style={styles.chatHeaderBanner}>
          <View style={styles.headerInfo}>
            <Text style={styles.headerPartnerName}>
              {otherUserName || 'Repair Specialist'}
            </Text>
            <View style={styles.statusDotRow}>
              <View style={styles.greenDot} />
              <Text style={styles.statusDotText}>Active Chat · Syncing live</Text>
            </View>
          </View>
        </View>
          ...
          ListEmptyComponent={
            <View style={styles.emptyChat}>
              ...
              <Text style={styles.emptySubtitle}>
                Say hello or ask any questions about your repair request!
              </Text>
            </View>
          }
        />
