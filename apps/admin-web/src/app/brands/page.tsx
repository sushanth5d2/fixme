...
export default function BrandsPage() {
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      await api.post('/brands', { name: newName.trim() });
      setNewName('');
      fetchBrands();
    } catch (err: any) {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Device Brands</h1>
        <p className="text-gray-500 mt-1">Manage device brands</p>
      </div>
          ...
          {brands.map((b) => (
            <tr key={b.id} className="hover:bg-gray-50">
              ...
              <td className="px-6 py-4">
                <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${b.isActive !== false ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
